...
async function saveSubmission(args) {
  ...
  const item = { id: uuidv4(), name, email, title, description, createdAt: now, updatedAt: now };
...
async function listSubmissions(args) {
  ...
  if (email) {
    ...
    querySpec = { query: "SELECT TOP 10 c.id, c.name, c.email, c.title, c.description, c.createdAt, c.updatedAt FROM c ORDER BY c.updatedAt DESC" };
  }

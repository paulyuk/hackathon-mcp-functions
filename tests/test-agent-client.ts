...
class AgentTestClient {
  ...
  /**
   * Get health status and verify Azure OpenAI configuration
   */
  async health(): Promise<any> {
    try {
      ...
      if (health.azureOpenAI) {
        console.log('🔑 Azure OpenAI Configuration:');
        console.log(`   - Endpoint: ${health.azureOpenAI.endpoint}`);
        console.log(`   - Deployment: ${health.azureOpenAI.deployment}`);
        console.log(`   - Using Identity: ${health.azureOpenAI.useIdentity}`);
        console.log(`   - Has Token: ${health.azureOpenAI.hasToken}`);
      }
...
async function runInteractiveTest() {
  ...
  // Test conversations with Azure OpenAI verification
  const testMessages = [
